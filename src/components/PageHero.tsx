    ...
    <section className="relative isolate min-h-[560px] overflow-hidden bg-black text-white md:min-h-[690px]">
      ...
      <div className="relative z-10 mx-auto flex min-h-[560px] w-full max-w-7xl flex-col justify-end px-5 pb-8 pt-28 md:min-h-[690px] md:px-8 md:pb-10">
        <div className="max-w-3xl" data-reveal-group>
          ...
          <h1 className="max-w-full break-words text-[2.65rem] font-semibold leading-[1.02] sm:text-5xl md:max-w-4xl md:text-7xl" data-reveal>
            {title}
          </h1>
          {subtitle ? (
            <p className="mt-5 text-xl font-medium text-white/90 md:text-3xl" data-reveal>
              {subtitle}
            </p>
          ) : null}
          {body ? (
            <p className="mt-5 max-w-[20rem] break-words text-base leading-8 text-white/82 sm:max-w-full md:max-w-2xl md:text-lg" data-reveal>
              {body}
            </p>
          ) : null}
        </div>
        {children ? <div className="mt-8" data-reveal-group>{children}</div> : null}
      </div>
    </section>
  );
}
